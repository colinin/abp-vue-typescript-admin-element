<template>
  <div class="app-container">
    ...
    <el-dialog
      :visible.sync="hasLoadPermission"
    ...
    >
      ...
      <span
        ...
        class="dialog-footer"
      >
        <el-button @click="hasLoadPermission = false">{{ $t('table.cancel') }}</el-button>
        <el-button
          type="primary"
...
import { createRole, CreateRoleDto, updateRole, deleteRole, getRoles, RoleDto, UpdateRoleDto } from '@/api/roles'
...
export default class extends Vue {
  ...
  private handleCreateRole() {
    this.$prompt(this.$t('roles.pleaseInputRoleName').toString(),
      ...
      createRole(createRoleDto).then(res => {
        const message = this.$t('roles.createRoleSuccess', { name: res.data.name }).toString()
  ...
  private handleSetDefaultRole(role: RoleDto, setDefault: boolean) {
    ...
    updateRole(role.id, setDefaultRoleDto).then(res => {
      this.$message.success(this.$t('roles.roleHasBeenSetDefault', { name: res.data.name }).toString())
      this.handleGetRoles()
    })
  }
